...
```{r}
bikeways <- st_read("networks/census_tracts_including_bikeways/census_tracts_including_bikeways.shp")
...
In our multivariate regression, we found that the only independent variable with a statistically significant confidence interval (95%) was binary_white, a categorical variable which refers to white majority census tracts. In other words, controlling for tenure, population, median income, and transport mode, we still found white majority census tracts to have a positive correlation with the number of feet of bike lane in a census tract. This means that majority white census tracts have 1408.03 more feet of bike lane than census tracts that are non-white majority.

On the other hand, our bivariate regression with binary_white (majority, non-majority census tract population) and sum_length (length of bike lane per census tract) found a stronger positive relationship with a higher level of confidence. When controlling for other variables, we found that relationship to be weaker. This relationship is still significant at 95% confidence, but previously was a 99% confidence. 
