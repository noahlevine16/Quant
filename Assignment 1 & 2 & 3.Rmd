...
-   Bike Racks (Number): Open Street Maps, amenity=bicycle_parking

Due to complications with our OSM data, our instructor recommended we continue with the assignment with solely ACS data, and return to our research question at a later date.
...
cont_summary <- tibble(
  Variable = c("Median Income", 
               "Transport (Bike)", "Median Age"),
  `Sample mean` = c(inc_t_test$estimate,
                    transport_t_test$estimate, age_t_test$estimate * 1000),
  `Population mean (95% confidence) - low` = 
    c(inc_t_test$conf.int[1],
      transport_t_test$conf.int[1], age_t_test$conf.int[1] * 1000),
  `Population mean (95% confidence) - high` =
    c(inc_t_test$conf.int[2],
      transport_t_test$conf.int[2], age_t_test$conf.int[2] * 1000),
  Median = c(inc_quartiles[3],
             transport_quartiles[3], age_quartiles[3] * 1000),
  `Interquartile range` = c(inc_quartiles[4] - inc_quartiles[2],
                            transport_quartiles[4] - transport_quartiles[2], age_quartiles[4] - age_quartiles[2] * 1000),
  `Standard deviation` = c(inc_st_dev,
                          transport_st_dev, age_st_dev * 1000))
...
pct_white_min <- t.test(acs_la_tracts_new$binary_white == "non-white")
```
...
Running correlation test between continuous variables. Until we get our bike data finalized, we will assume income is our dependant variable.

```{r}
cor.test(~ med_income_E + med_age_E, data = acs_la_tracts_new2)

```
In running a bivariate analysis between these two continious variables, our 95% confidence interval does not include zero, and all valuables are positive. This leads us to conclude, with 95% confidence, that the higher median ages as associated with higher levels of income. 

Below we try the same method, but with a regression.

```{r}
age_model <- lm(med_income_E ~ med_age_E, data = acs_la_tracts_new2)
summary(age_model)
```
Our p value here is 2e-16 rather than 2.23-16 (something we may want to clarify with the TAs or Carole), but also need to confirm if the R squared value of .3278 indeed the square of .573.

## Density 

We also choose to run an alaysis of median income with population density, to see if areas of higher income have less people living in that census tract. 

```{r}
cor.test(~ med_income_E + pop_E, data = acs_la_tracts_new2)

```
The P value does not equal zero, and the 95% confidence interval is all negative values. With the correlation coefficient being negative, we could say with 95% confidence that as the median income of a census tract does increase, the population of the census track decreases. 

```{r}
dens_model <- lm( med_income_E ~ pop_E, data = acs_la_tracts_new2)
summary(dens_model)
```
...
Until we can get our bike lane data clipped in ArcGis, we are presently curious in assessing if LA City tracks that are catorigized as white majority have higher levels of income than non-white majority tracts.

```{r}
t.test(med_income_E ~ binary_white, data = acs_la_tracts_new2)

```
In the above t-test, the 95% confidence interval is a very high negative number range that does not cross zero, and the p value is very small, indicating a strong correlation. We can say, with 95% confidence, that the mean median income in the group of non-white census tracts if 24,756.54 USD and the mean median income estimate in the group of white majority census tracts is 41,965.89 USD. This is a start confidence 

We can run the same result as a regression 

```{r}
race_income_model <- lm(med_income_E ~ binary_white, data = acs_la_tracts_new2)

summary(race_income_model)
```
The adjusted R squared in the linear regression does match the correlation coefficient in the above. 

## Effect of Race on median income

Where we are having issues!!! WE can also use regression to find the effect of race (% white) on income or try to access the other cateorical variable of % residnets new, but we need to further categorize that. 


data <- data %>%
  mutate(binary_white = as.factor(binary_white)) %>%
  mutate(binary_white = relevel(binary_white, "white"))

binary_white_model <- lm(med_income_E ~ binary_white, acs_la_tracts_new2)
